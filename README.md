...
| `http_req_receiving`  | Time spent receiving the response from the server.                                                                                                                                        | High values can indicate issues with the TLS configuration, network latency, or server-side processing of TLS connections.                         | `http_req_sending` | Time taken to send the HTTP request to the server. | This is usually short. Longer times could indicate large request bodies or network issues. | `http_req_tls_handshake` | Time spent performing the TLS handshake. If the connection is already established and the handshake is not this will be zero. | High values can indicate issues with the TLS configuration, network latency, or server-side processing of TLS connections. | `http_req_waiting` | Time spent waiting for a response form server after sending the request. | This is often the largest part of the request duration and can be affected by network latency or server processing time. | `http_reqs` | The time spent establishing a new TCP connection. If a connection is reused(like in keep-alive), this will be zero. | High values could indicate network issues or server side limitations in handling new connections. |
